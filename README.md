...
**⑩ かな漢字変換のし忘れ（全部ひらがな → 漢字に）**
```
入力: あした、かいしゃのかいぎしつでうちあわせをします。しりょうをじゅんびしてください。
出力: 明日、会社の会議室でお打ち合わせを行います。資料を用意してください。
```

**⑪ ローマ字残り＋崩れ**
```
入力: おはようgザimasu。kyouのかいぎyoroshikuおねがいします。
出力: おはようございます。今日の会議、よろしくお願いいたします。
```

**⑫ ひどい混在（ローマ字＋誤字＋時刻）でも大筋は直る**
```
入力: おせわになてまうす。ashitaのkaigi、jikan11:00→14:00にへんこうおねがいしす。shiryouはmaeもってkyouyuuしときます。
出力: お世話になってます。明日の会議、時間 11:00〜14:00 に変更お願いいたします。資料は先もって来週提出します。
```
> ほぼ復元できているが、「**共有**しときます」が「**来週提出**します」に化ける誤りも残る（崩れが激しいほど一部は外す → 下の「苦手な例」へ）。
...
### 苦手な例・限界（崩れが激しいと外すことがある）

入力が極端に崩れていると、もっともらしいが誤った補正になることがあります。あえて失敗例も載せます
（これらも `qwen3.5-jp-4b:q6` の実出力）。

**A. 二重否定 → 意味が反転してしまう**
```
入力: 別に行きたくないわけじゃないんだけど、ちょっとむずかしそう。
出力: 別に行きたいわけじゃないんだけど、ちょっと難しそう。
```
> ✗ 「行きたく**ない**わけじゃない（＝本当は行きたい）」が「行きたいわけじゃない（＝行きたくない）」へ反転。二重否定は苦手。

**B. 全ひらがな → もっともらしい誤変換**
```
入力: せんじつわうちあわせのおじかんいただきありがとうございました
出力: 先日、お打ち合わせの機会をいただきありがとうございました。
```
> ✗ 「お**じかん**（お時間）」が「機会」に化ける。漢字の手がかりが無いと推測を外しやすい。

**C. 崩れすぎ → 内容が別物に**
```
入力: おせわになておりまうす。れんらくありがとうごzaいます。あとでれんらくしまうす。
出力: お世話になっております。よろしくお願いいたします。あとで連絡いたします。
```
> ✗ 「連絡ありがとうございます」が消えて別の挨拶に置き換わった（ローマ字混入「ごza」が引き金）。

**D. 脱字が多い → 語が落ちる**
```
入力: あすのかいぎ、しりょうつくとおいてくらさい。
出力: 明日の会議、資料を置いてください。
```
> ✗ 「**作って**おいて」の「作る」が抜けて「置いて」だけに。

**E. 全ひらがなの業務用語 → 壊滅的に誤読**
```
入力: らいげつのよさんについて、けいりぶにかくにんします。
出力: ライジェツのようさんについて、経歴をかくにんします。
```
> ✗ 「来月／予算／経理部」が「ライジェツ／ようさん／経歴」に化け、「かくにん」も未変換。漢字の手がかりが無い業務用語は特に苦手。

**F. ローマ字の同音語を取り違える**
```
入力: ごめんkyou tukareta…mata ashita renraku surune
出力: ごめん、今日別れた…。また明日連絡するね。
```
> ✗ 「tukareta（疲れた）」を「別れた」と誤読。ローマ字は綴りが同じ別語に化けやすい。

> ⚠️ いずれもローカル軽量4Bモデルゆえの限界です。**本ツールは非破壊**（本文は書き換えない）なので、
> 結果は必ず原文と並べて確認し、人間が最終判断してから貼り付けてください。崩れが激しい入力ほど要注意です。
